...
"""
import logging
from multiprocessing import Process, Manager, current_process, Queue, Pool, cpu_count
...
def factorize(*numbers) -> []:
    ...
    with Manager() as manager:
        ...
        with Pool(processes=cpu_count()) as pool:
            pool.starmap(count, [(num, result_arr) for num in numbers])

        logger.debug(f'{result_arr}')
        return list(result_arr)


def count(num: int, result_arr: list):
    """
